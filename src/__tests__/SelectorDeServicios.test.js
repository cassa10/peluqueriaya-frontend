  ...
  describe("cuando el selector de servicios se monta correctamente", () => {
    ...
    it("sin servicios y con corteMin 100.55, se muestra el servicio básico unicamente con el texto $100.55", () => {
      ...
      const component = shallow(
        <SelectorDeServicios
          servicios={servicios}
          handleChecked={handleCheckedFn}
          corteMin={corteMin}
        />
      );
      expect(existeListItem(component, "corteMin")).toBeTruthy();
      expect(textoDescripcionListItem(component, "corteMin")).toEqual(
        "Servicio básico"
      );
      expect(textoPrecioListItem(component, "corteMin")).toEqual(
        formatPrice(100.55)
      );
    });
    ...
    it("con los tres servicios y corteMin 200, se muestra el servicio básico con precio $200 con el texto $200.00 y los servicios con sus nombres y precios", () => {
      ...
      assertExistenTodosLosListItemsYSusRespectivasDescripcionesYPrecios(
        component,
        servicios
      );
    });
    ...
    describe("con el servicio básico y los tres servicios mostrandose", () => {
      it("por defecto solo el servicio básico esta seleccionado", () => {
        ...
        expect(
          listItemCheckbox(component, "corteMin").prop("disabled")
        ).toBeTruthy();
        assertTodosLosServiciosExceptoElBasicoNoEstanSeleccionados(
          component,
          servicios
        );
      ...
      it("cuando se hace click en el primer servicio (Servicio 1), este se selecciona y llama a la funcion handleCheckedFn con una lista con los servicios seleccionados como argumento", () => {
        ...
        expect(
          firstArgumentOfFirstCallOfAFunction(handleCheckedFn)
        ).toStrictEqual([primerServicio]);

        expect(
          listItemCheckbox(component, primerServicio.id).prop("checked")
        ).toBeTruthy();
        expect(
          listItemCheckbox(component, servicios[1].id).prop("checked")
        ).toBeFalsy();
        expect(
          listItemCheckbox(component, servicios[2].id).prop("checked")
        ).toBeFalsy();
      });
      it("cuando se hace click en cada servicio, estos se seleccionan y llama a la funcion handleCheckedFn las veces que fueron seleccionados los servicios con una lista con los que fueron seleccionados en el orden que fueron llamados como argumento", () => {
        ...
        expect(handleCheckedFn.mock.calls).toHaveLength(1);
        expect(
          firstArgumentOfNCallOfAFunction(handleCheckedFn, 0)
        ).toStrictEqual([servicios[0]]);

        listItemToBeClicked(component, servicios[1].id).props().onClick();
        expect(handleCheckedFn.mock.calls).toHaveLength(2);
        expect(
          firstArgumentOfNCallOfAFunction(handleCheckedFn, 1)
        ).toStrictEqual([servicios[0], servicios[1]]);

        listItemToBeClicked(component, servicios[2].id).props().onClick();
        expect(handleCheckedFn.mock.calls).toHaveLength(3);
        expect(
          firstArgumentOfNCallOfAFunction(handleCheckedFn, 2)
        ).toStrictEqual(servicios);

        expect(
          listItemCheckbox(component, servicios[0].id).prop("checked")
        ).toBeTruthy();
        expect(
          listItemCheckbox(component, servicios[1].id).prop("checked")
        ).toBeTruthy();
...
const obtainDataTestIdInFindString = (keyDataTestId) =>
  `[data-testid="${keyDataTestId}"]`;

const existeListItem = (component, id) =>
  component.find(obtainDataTestIdInFindString(`list_item_${id}`)).exists();
const textoDescripcionListItem = (component, id) =>
  component
    .find(obtainDataTestIdInFindString(`checkbox-list-label-${id}-text`))
    .prop("primary");
const textoPrecioListItem = (component, id) =>
  component
    .find(obtainDataTestIdInFindString(`checkbox-list-label-${id}-text-price`))
    .prop("primary");
const listItemCheckbox = (component, id) =>
  component.find(obtainDataTestIdInFindString(`checkbox-list-label-${id}`));
const listItemToBeClicked = (component, id) =>
  component.find(obtainDataTestIdInFindString(`list-item-${id}`));
...
const assertExisteServicioMinListItemYSuRespectivaDescripcionYPrecio = (
  component,
  servicio
) => {
  expect(existeListItem(component, servicio.id)).toBeTruthy();
  expect(textoDescripcionListItem(component, servicio.id)).toEqual(
    servicio.nombre
  );
  expect(textoPrecioListItem(component, servicio.id)).toEqual(
    formatPrice(servicio.precio)
  );
};

const assertExistenTodosLosListItemsYSusRespectivasDescripcionesYPrecios = (
  component,
  servicios
) => {
  servicios.map((servicio) =>
    assertExisteServicioMinListItemYSuRespectivaDescripcionYPrecio(
      component,
      servicio
    )
  );
...
const assertTodosLosServiciosExceptoElBasicoNoEstanSeleccionados = (
  component,
  servicios
) => {
  servicios.map((servicio) =>
