...
const PaginaGestionCliente = () => {
  ...
  const handleShowClientInfo = (sonTurnosActuales, turno) => {
    return sonTurnosActuales && mostrarRowsInfoCliente(turno);
  };

  const showFechaFin = (turno) =>
    turno.estaCancelado
      ? `${formatDate(turno.fechaCancelacion)} ${formatTime(
          turno.fechaCancelacion
        )}`
      ...
      <TableBody>
        {turnos.map((turno) => (
          ...
          >
            ...
            {handleShowDataInRow(
              !isTurnosSelected,
              handlePuntuacionPorEstado(turno)
            )}
          </TableRow>
        ))}
      </TableBody>
  ...
  const handleShowPaginationButtons = () => {
    return (!cargandoTurnos || cargandoChangePage) && showPaginationButtons();
  };
